...
func configureDBPool(db *gorm.DB, envPrefix string, defaultIdle, defaultOpen int) error {
	sqlDB, err := db.DB()
	if err != nil {
		return err
	}
	maxOpen := common.GetEnvOrDefault(envPrefix+"_MAX_OPEN_CONNS", defaultOpen)
	maxIdle := common.GetEnvOrDefault(envPrefix+"_MAX_IDLE_CONNS", defaultIdle)
	if maxOpen > 0 && maxIdle > maxOpen {
		maxIdle = maxOpen
	}
	sqlDB.SetMaxIdleConns(maxIdle)
	sqlDB.SetMaxOpenConns(maxOpen)
	sqlDB.SetConnMaxLifetime(time.Second * time.Duration(common.GetEnvOrDefault(envPrefix+"_MAX_LIFETIME", 60)))
	return nil
}
...
func InitDB() (err error) {
	...
	if err == nil {
		...
		if err := configureDBPool(DB, "SQL", 10, 50); err != nil {
			return err
		}
...
func InitLogDB() (err error) {
	...
	if err == nil {
		...
		if err := configureDBPool(LOG_DB, "LOG_SQL", 5, 20); err != nil {
			return err
		}
